...
export class TinyInfo {
  ...
  async getPaginasProdutos(situacao = null) {
    let page = 1;
    let data = [
      { key: "pesquisa", value: "" },
    ...
    if (situacao) {
      data.push({ key: "situacao", value: situacao });
    }
  ...
  async getPaginasContasReceber(
    data_ini_vencimento = null,
    numero_doc = null,
    situacao = "aberto"
  ) {
    let page = 1;
    let data = [{ key: "pagina", value: page }];

    if (data_ini_vencimento)
      data.push({ key: "data_ini_vencimento", value: data_ini_vencimento });
    if (situacao) data.push({ key: "situacao", value: situacao });
    if (numero_doc) data.push({ key: "numero_doc", value: numero_doc });
